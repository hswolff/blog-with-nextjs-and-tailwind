...
import { getAllPosts } from '../lib/experiments';

export default function Experiments({ posts }) {
  return (
    <div>
      <Head>
      ...
      <div className="space-y-4">
        {posts.map((item) => (
          <ExperimentsListItem key={item.slug} {...item} />
        ))}
      </div>
    </div>
  );
...
function ExperimentsListItem({ slug, title, date, content }) {
  return (
